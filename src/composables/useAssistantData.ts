...
import type { DocumentListItem, UIDesign, ProjectFile, FileTreeEntry, TaskCacheItem } from '~/types/assistant'
import type { DesignNode } from '~/types/design'
import { registerDesign } from '~/composables/useCanvasContext'
...
export function useAssistantData(deps: AssistantDataDeps) {
  const { projectStore, sendRequest, callTool, connected, busTasksCache, codeEditorState } = deps
    ...
    async (projectId) => {
      if (projectId) {
        ...
        try {
          ...
          console.warn('[AssistantData] Failed to load designs:', e)
        }
