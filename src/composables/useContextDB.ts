...
export function useContextDB() {
  ...
  const { isTauri, connected } = contextService
  ...
  async function pinnedList(): Promise<PinnedItem[]> {
    ...
    try {
      const result = await contextService.sendRequest<{ items?: PinnedItem[] }>('pinned.list', {})
      return result?.items || []
    } catch (error) {
  ...
  async function pinnedAdd(item: PinnedItem): Promise<boolean> {
    ...
    try {
      await contextService.sendRequest('pinned.add', { ...item })
      return true
    } catch (error) {
  ...
  async function pinnedRemove(id: string): Promise<boolean> {
    if (!await ensureConnected()) return false
    try {
      await contextService.sendRequest('pinned.remove', { id })
      return true
    } catch (error) {
  ...
  async function pinnedReorder(orderedIds: string[]): Promise<boolean> {
    if (!await ensureConnected()) return false
    try {
      await contextService.sendRequest('pinned.reorder', {
  ...
  async function designList(projectId?: number): Promise<ContextUIDesign[]> {
    ...
    try {
      const result = await contextService.sendRequest<{ designs?: ContextUIDesign[] }>('designs.list', {
        projectId
      })
  ...
  async function designGet(localId: string): Promise<ContextUIDesign | null> {
    ...
    try {
      const result = await contextService.sendRequest<{ design?: ContextUIDesign }>('designs.get', {
        localId
      })
  ...
  async function designSave(design: Partial<ContextUIDesign> & { localId: string; name: string }): Promise<number | null> {
    if (!await ensureConnected()) return null
    try {
      const result = await contextService.sendRequest<{ id: number; localId: string }>('designs.save', design as unknown as Record<string, unknown>)
  ...
  async function designDelete(localId: string): Promise<boolean> {
    if (!await ensureConnected()) return false
    try {
      await contextService.sendRequest('designs.delete', { localId })
      return true
    } catch (error) {
  ...
  async function kvGet(key: string): Promise<string | null> {
    if (!await ensureConnected()) return null
    try {
      const result = await contextService.sendRequest<{ value?: string }>('kv.get', { key })
  ...
  async function kvSet(key: string, value: string, category?: string): Promise<boolean> {
    if (!await ensureConnected()) return false
    try {
      await contextService.sendRequest('kv.set', { key, value, category })
      return true
    } catch (error) {
  ...
  async function kvDelete(key: string): Promise<boolean> {
    if (!await ensureConnected()) return false
    try {
      await contextService.sendRequest('kv.delete', { key })
      return true
    } catch (error) {
  ...
  async function kvList(category?: string): Promise<KVEntry[]> {
    if (!await ensureConnected()) return []
    try {
      const result = await contextService.sendRequest<{ entries?: KVEntry[] }>('kv.list', { category })
  ...
  async function settingGet(key: string): Promise<string | null> {
    if (!await ensureConnected()) return null
    try {
      const result = await contextService.sendRequest<{ value?: string }>('settings.get', { key })
      return result?.value ?? null
    } catch (error) {
  ...
  async function settingSet(key: string, value: string): Promise<boolean> {
    if (!await ensureConnected()) return false
    try {
      await contextService.sendRequest('settings.set', { key, value })
      return true
    } catch (error) {
