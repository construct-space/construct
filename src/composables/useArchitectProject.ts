...
export function useArchitectProject(deps: ProjectDeps) {
  ...
  function wantsComprehensiveDocs(currentPlan: ArchitectPlan): boolean {
    ...
    const planText = [
      ...
      prd?.overview || '',
      (prd as Record<string, unknown>)?.techRationale as string || '',
      (prd as Record<string, unknown>)?.designNotes as string || '',
      ...(prd?.mvpScope || []),
      ...(prd?.futureConsiderations || []),
    ]
      .join(' ')
      .toLowerCase()
  ...
  function getSelectedDocsForKickoff(currentPlan: ArchitectPlan): DocumentType[] {
    ...
    if (docsDepth.includes('full') || docsDepth.includes('comprehensive') || docsDepth.includes('complete')) {
      return ['prd', 'readme', 'architecture', 'data-models', 'ui-spec', 'roadmap', 'ai-context', 'setup']
    } else if (docsDepth.includes('readme') && !docsDepth.includes('prd') && !docsDepth.includes('standard')) {
      return ['readme']
    }

    // When rich docs are present, include all available doc types
    if (currentPlan.docs) {
      const selected = new Set<DocumentType>(['readme', 'prd', ...getDefaultDocuments(currentPlan)])
      if (currentPlan.docs.dataModels) selected.add('data-models')
      if (currentPlan.docs.uiSpec) selected.add('ui-spec')
      if (currentPlan.docs.aiContext) selected.add('ai-context')
