...
export interface ProjectConfig {
  ...
  designs: Array<{
    name: string
    path: string           // Relative to project/design/
    type: 'ui' | 'ux'
  }>
...
EOFCONFIG`], projectPath)
  ...
  /**
   * Create a new design file in the project
   */
  const createDesignFile = async (
    ...
    try {
      ...
      if (!designExists) {
        await tauriFs.mkdir(designPath)
      }

      // Add extension if not present
      const ext = type === 'ui' ? '.ui' : '.ux'
      const fullName = fileName.endsWith(ext) ? fileName : `${fileName}${ext}`
      const filePath = `${designPath}/${fullName}`

      // Create empty design file
      const emptyDesign = type === 'ui'
        ? {
            version: 1,
            type: 'design',
      ...
      console.error('Failed to create design file:', e)
      return null
    }
  }
