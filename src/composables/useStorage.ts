...
export interface UseStorageReturn {
  ...
  // UI Designs (IndexedDB replacement)
  designs: {
    list: (projectId?: number) => Promise<UIDesign[]>
    get: (localId: string) => Promise<UIDesign | null>
    save: (design: UIDesign) => Promise<{ id: number; localId: string }>
...
export function useStorage(): UseStorageReturn {
  const { sendRequest, isTauri } = useContextService()
  ...
  async function get<T = unknown>(key: string, options?: StorageOptions): Promise<T | null> {
    ...
    try {
      const result = await sendRequest<{ value: T | null }>('storage.get', {
        key,
  ...
  async function remove(key: string, options?: StorageOptions): Promise<void> {
    ...
    try {
      await sendRequest('storage.delete', {
        key
      })
    } catch (error) {
  ...
  async function getMany<T = unknown>(keys: string[], options?: StorageOptions): Promise<Record<string, T>> {
    ...
    try {
      const result = await sendRequest<{ items: Record<string, T> }>('storage.batch_get', {
        keys
      })
      return result?.items ?? {}
    } catch (error) {
  ...
  async function setMany(items: Record<string, unknown>, options?: StorageOptions): Promise<void> {
    ...
    try {
      await sendRequest('storage.batch_set', {
        items,
        category: options?.category
      })
    } catch (error) {
  ...
  async function list(options?: StorageOptions): Promise<Array<{ key: string; value: unknown }>> {
    ...
    try {
      const result = await sendRequest<{ items: Array<{ key: string; value: unknown }> }>('storage.list', {
        category: options?.category,
        projectId: options?.projectId,
        userId: options?.userId
      })
  ...
  const designs = {
    async list(projectId?: number): Promise<UIDesign[]> {
      if (!isTauri.value) {
        // Fallback: return from localStorage
        const items = await list({ category: 'designs', projectId })
        return items.map(item => deserializeDesign(item.value as Record<string, unknown>))
      }

      try {
        const result = await sendRequest<{ designs: UIDesign[] }>('designs.list', {
          projectId: projectId ?? null
        })
        return (result?.designs ?? []).map(d => deserializeDesign(d as unknown as Record<string, unknown>))
      } catch (error) {
        console.error('[useStorage] designs.list error:', error)
        return []
      }
    },

    async get(localId: string): Promise<UIDesign | null> {
      if (!isTauri.value) {
        const value = await get<Record<string, unknown>>(`design_${localId}`, { category: 'designs' })
        return value ? deserializeDesign(value) : null
      }

      try {
        const result = await sendRequest<UIDesign | null>('designs.get', {
          localId
        })
        return result ? deserializeDesign(result as unknown as Record<string, unknown>) : null
      } catch (error) {
        console.error('[useStorage] designs.get error:', error)
        return null
      }
    },
    ...
    async save(design: UIDesign): Promise<{ id: number; localId: string }> {
      ...
      try {
        const result = await sendRequest<{ id: number; localId: string }>('designs.save',
          serializeDesign(designToSave)
        )
    ...
    async delete(localId: string): Promise<void> {
      if (!isTauri.value) {
        await remove(`design_${localId}`, { category: 'designs' })
        return
      }

      try {
        await sendRequest('designs.delete', { localId })
      } catch (error) {
        console.error('[useStorage] designs.delete error:', error)
      }
    }
  }
  ...
  const projectSettings = {
    async get(projectId: number): Promise<ProjectLocalSettings | null> {
      ...
      try {
        const result = await sendRequest<ProjectLocalSettings | null>('project_settings.get', {
          projectId
        })
        return result ?? null
      } catch (error) {
    ...
    async set(projectId: number, settings: Partial<ProjectLocalSettings>): Promise<void> {
      ...
      try {
        await sendRequest('project_settings.set', {
          projectId,
  ...
  const pinned = {
    async list(): Promise<PinnedItem[]> {
      ...
      try {
        const result = await sendRequest<{ items: PinnedItem[] }>('pinned.list', {})
        return result?.items ?? []
      } catch (error) {
    ...
    async add(item: PinnedItem): Promise<void> {
      ...
      try {
        await sendRequest('pinned.add', pinnedItem as unknown as Record<string, unknown>)
      } catch (error) {
    ...
    async remove(id: string): Promise<void> {
      ...
      try {
        await sendRequest('pinned.remove', { id })
      } catch (error) {
    ...
    async reorder(items: Array<{ id: string; sortOrder: number }>): Promise<void> {
      ...
      try {
        await sendRequest('pinned.reorder', { items })
      } catch (error) {
...
function serializeDesign(design: UIDesign): Record<string, unknown> {
  return {
    ...design,
    createdAt: design.createdAt instanceof Date ? design.createdAt.toISOString() : design.createdAt,
    updatedAt: design.updatedAt instanceof Date ? design.updatedAt.toISOString() : design.updatedAt
  }
}
