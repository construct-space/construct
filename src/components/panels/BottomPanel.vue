    ...
    <div class="flex items-center justify-between h-9 px-2 bg-white/5">
      ...
      <div class="flex items-center gap-0.5">
        <button
          v-for="tab in tabs"
          :key="tab.id"
          class="flex items-center gap-1.5 px-3 h-7 text-xs rounded-md transition-colors"
        ...
        >
          ...
          <span>{{ tab.label }}</span>
          <span
            v-if="tab.id === 'problems' && problems.length > 0"
      ...
      <div class="flex items-center gap-1">
        <!-- Running indicator -->
        <div
          v-if="isRunning && processName"
          class="flex items-center gap-1.5 px-2 h-6 rounded-md mr-2"
          :class="statusBgColor"
        >
          <span
        ...
        <template v-if="isRunning">
          <Tooltip text="Hot Reload (r)">
            <button
              class="p-1.5 rounded-md text-yellow-400 hover:bg-yellow-500/10 transition-colors"
          ...
          <Tooltip text="Restart (R)">
            <button
              class="p-1.5 rounded-md text-orange-400 hover:bg-orange-500/10 transition-colors"
        ...
        <Tooltip text="Clear">
          <button
            class="p-1.5 rounded-md text-app-muted hover:text-app hover:bg-white/5 transition-colors"
            @click="emit('clear')"
          >
    ...
    <div class="flex-1 min-h-0 overflow-hidden">
      ...
      >
        ...
        <pre
          v-if="debugOutput"
          class="whitespace-pre-wrap break-words"
          v-html="colorizeOutput(debugOutput)"
        />
        <!-- eslint-enable vue/no-v-html -->
      ...
      <div v-show="currentTab === 'problems'" class="h-full overflow-auto">
        ...
        <div v-else class="divide-y divide-white/10">
          ...
          >
            <Icon
              :name="problem.type === 'error' ? 'i-lucide-x-circle' :
                     problem.type === 'warning' ? 'i-lucide-alert-triangle' :
                     'i-lucide-info'"
              class="size-4 mt-0.5 flex-shrink-0"
              :class="problem.type === 'error' ? 'text-red-400' :
