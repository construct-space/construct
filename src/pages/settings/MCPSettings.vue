...
async function loadServers() {
  ...
  try {
    const result = await contextService.sendRequest('mcp.list', {}) as { servers?: MCPServer[] }
...
async function toggleServer(server: MCPServer) {
  try {
    if (server.enabled) {
      await contextService.sendRequest('mcp.disable', { id: server.id })
      toast.add({ title: `${server.name} disabled`, color: 'warning' })
    } else {
      await contextService.sendRequest('mcp.enable', { id: server.id })
...
async function addServer() {
  ...
  try {
    ...
    await contextService.sendRequest('mcp.add', payload)
...
async function removeServer(server: MCPServer) {
  try {
    await contextService.sendRequest('mcp.remove', { id: server.id })
...
async function testServer(server: MCPServer) {
  try {
    await contextService.sendRequest('mcp.test', { id: server.id })
