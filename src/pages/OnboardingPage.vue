...
const anthropicAuthCode = ref('')
const showAnthropicAuthCodeInput = ref(false)
...
async function startAnthropicAuth() {
  try {
    ...
    showAnthropicAuthCodeInput.value = true
    try {
...
async function submitAnthropicCode() {
  if (!anthropicAuthCode.value) return
  try {
    const success = await anthropicOAuth.exchangeCode(anthropicAuthCode.value)
    if (success) {
      showAnthropicAuthCodeInput.value = false
      anthropicAuthCode.value = ''
      toast.add({ title: 'Claude Max connected', color: 'success' })
    } else {
...
async function startOpenAIAuth() {
  ...
  try {
    const result = await contextService.sendRequest('auth.openai.start', {}) as { auth_url?: string; authenticated?: boolean }
    if (result?.authenticated) {
      await checkOpenAIStatus()
      toast.add({ title: 'OpenAI connected', color: 'success' })
      return
    }
    if (!result?.auth_url) throw new Error('No auth URL')
...
const connectedProviderCount = computed(() => {
  let count = 0
  if (anthropicOAuth.isAuthenticated.value) count++
          ...
          <div class="space-y-4 mb-6">
            ...
            <div class="p-4 rounded-xl border border-[var(--app-border)]">
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    <p class="text-sm font-medium text-[var(--app-foreground)]">Claude Max</p>
                    <p class="text-xs text-[var(--app-muted)]">Unlimited Claude with Pro/Max subscription</p>
                  </div>
                </div>
                <span
                  v-if="anthropicOAuth.isAuthenticated.value"
                  class="px-2 py-0.5 text-xs rounded-full bg-green-500/10 text-green-500"
                >
                  Connected
                </span>
                <button
                  v-else-if="!showAnthropicAuthCodeInput"
                  class="px-3 py-1.5 text-xs rounded-lg bg-app-accent text-white hover:opacity-90 transition-opacity"
                  @click="startAnthropicAuth"
                >
                  Connect
                </button>
              </div>
              <!-- Auth code input -->
              <div v-if="showAnthropicAuthCodeInput && !anthropicOAuth.isAuthenticated.value" class="mt-3 flex gap-2">
                <Input v-model="anthropicAuthCode" placeholder="Paste authorization code..." size="sm" class="flex-1" />
                <Button size="sm" :loading="anthropicOAuth.isLoading.value" :disabled="!anthropicAuthCode" label="Submit" @click="submitAnthropicCode" />
                <Button size="sm" variant="ghost" label="Cancel" @click="showAnthropicAuthCodeInput = false; anthropicAuthCode = ''" />
              </div>
            </div>
            ...
            <div class="p-4 rounded-xl border border-[var(--app-border)]">
              ...
              <div v-if="showOpenAIAuthCodeInput && !openAIAuthenticated" class="mt-3 flex gap-2">
                ...
                <Button size="sm" variant="ghost" label="Cancel" @click="showOpenAIAuthCodeInput = false; openAIAuthCode = ''" />
              </div>
            </div>
