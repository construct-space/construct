...
export function useChatRoomAgent() {
  ...
  async function ensureProjectDocsLoaded(): Promise<void> {
    const rawId = projectStore.currentProject?.id
  ...
  watch(() => projectStore.currentProject?.id, async (rawId) => {
    projectDocs.value = []
  ...
  function buildScopedContext(full: SpaceContext): Partial<SpaceContext> {
    const allowedSpaces = new Set<string>((projectStore as any).accessibleSpaces ?? ['code', 'ui', 'kanban', 'notes', 'git', 'docs', 'chat'])
    const scoped: Partial<SpaceContext> = {
      activeSpace: 'chat',
      ...((full as any).chat ? { chat: (full as any).chat } : {}),
    }
  ...
  function buildScopedContextSummary(scoped: Partial<SpaceContext>): string {
    ...
    if ((scoped as any).chat?.activeConversation) {
      lines.push(`Chat in focus: ${(scoped as any).chat.activeConversation}`)
    }
  ...
  async function generateReply(params: {
    ...
    try {
      ...
      await chatStream({
        ...
        local_data: {
          security_scope: {
            policy: (authStore as any).roleId ? 'role_based' : 'personal',
            ...((authStore as any).roleId ? { role_id: (authStore as any).roleId } : {}),
            ...((authStore as any).hasFullAccess !== undefined ? { is_company_owner: (authStore as any).hasFullAccess } : {}),
            allowed_spaces: (projectStore as any).accessibleSpaces ?? ['code', 'ui', 'kanban', 'notes', 'git', 'docs', 'chat'],
          },
